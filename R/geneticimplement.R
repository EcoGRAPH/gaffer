geneticimplement <- function(individpergeneration = NULL,
  ...
  if (option == 'yes') {
    ...
    # this next line shouldn't be hardcoded
    envnames <- envnames[!(envnames %in% c("placeid", "date", "year", "doy"))]
    modelsdf <- Genmodelsdf

    print("You selected to run genetic algoithm from some generations")

    print(head(modelsdf))

    # save the first generation
    write.csv(modelsdf, paste(".\\outputs\\generation_",
                              tail(Genmodelsdf,1)$generation, ".csv",
                              sep=""))
    ...
    # run the rest of the generations
    for (generation in tail(Genmodelsdf,1)$generation + 1 :generations) {

      print(tail(Genmodelsdf,1)$generation + 1)
    ...
    for (i in 1:length(gens)){
      ...
      if (count == 2){
        ...
        plot(head(gens,2),head(measure,2),type="l", main="generations vs measure",xlab = "Generations",
             ylab="Model min measure")
        # print(gens[1:2])
        # print(measure[1:2])
        gens <- gens[-seq(1:2)]
        measure <- measure[-seq(1:2)]
