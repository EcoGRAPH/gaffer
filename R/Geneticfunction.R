...
)
{
  ...
  #print(bestmodel)
  write.csv(bestmodel %>% dplyr::select(!starts_with("clustermat", ignore.case = TRUE)),
            paste("Best_model.csv",
                  sep=""))
  ...
  #print(adjacency2)
  bestmod$cluster = adjacency2$fillcluster
  #print(bestmod)
  write.csv(bestmod %>% dplyr::select(!starts_with("clustermat", ignore.case = TRUE)),
            paste("Region_Clusters.csv",
                  sep=""))
